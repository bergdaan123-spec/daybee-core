'use client'

import React, { useState } from 'react'
import type { Identity } from '@prisma/client'
import { useFetch, apiFetch } from '@/hooks/useApi'
import Modal from '@/components/ui/Modal'
import Card from '@/components/ui/Card'
import PageShell from '@/components/layout/PageShell'
import IdentityTable from './IdentityTable'
import IdentityForm, { type IdentityFormValues } from './IdentityForm'
import { primaryButtonStyle, errorBoxStyle } from '@/styles/shared'
import { Plus } from 'lucide-react'

type ModalMode = 'create' | 'edit' | 'delete' | null

export default function IdentitiesClient() {
  const { data: identities, loading, error: fetchError, refetch } =
    useFetch<Identity[]>('/api/identities')

  const [mode, setMode]         = useState<ModalMode>(null)
  const [selected, setSelected] = useState<Identity | null>(null)
  const [saving, setSaving]     = useState(false)
  const [formError, setFormError] = useState<string | null>(null)

  function openCreate()              { setSelected(null);     setFormError(null); setMode('create') }
  function openEdit(i: Identity)     { setSelected(i);        setFormError(null); setMode('edit')   }
  function openDelete(i: Identity)   { setSelected(i);        setFormError(null); setMode('delete') }
  function closeModal()              { setMode(null); setSelected(null); setFormError(null) }

  async function handleCreate(values: IdentityFormValues) {
    setSaving(true); setFormError(null)
    const { error } = await apiFetch('/api/identities', {
      method: 'POST', body: JSON.stringify(values),
    })
    setSaving(false)
    if (error) { setFormError(error); return }
    closeModal(); refetch()
  }

  async function handleEdit(values: IdentityFormValues) {
    if (!selected) return
    setSaving(true); setFormError(null)
    const { error } = await apiFetch(`/api/identities/${selected.id}`, {
      method: 'PUT', body: JSON.stringify(values),
    })
    setSaving(false)
    if (error) { setFormError(error); return }
    closeModal(); refetch()
  }

  async function handleDelete() {
    if (!selected) return
    setSaving(true); setFormError(null)
    const { error } = await apiFetch(`/api/identities/${selected.id}`, { method: 'DELETE' })
    setSaving(false)
    if (error) { setFormError(error); return }
    closeModal(); refetch()
  }

  const initialValues: Partial<IdentityFormValues> | undefined = selected
    ? {
        name:      selected.name,
        type:      selected.type as 'PRIVATE' | 'BV',
        address:   selected.address,
        postalCode: selected.postalCode,
        city:      selected.city,
        country:   selected.country,
        email:     selected.email,
        phone:     selected.phone      ?? '',
        kvkNumber: selected.kvkNumber  ?? '',
        btwNumber: selected.btwNumber  ?? '',
        iban:      selected.iban       ?? '',
      }
    : undefined

  return (
    <>
      <PageShell
        title="Identiteiten"
        description="Beheer de afzenderidentiteiten die op uw huurfacturen worden gebruikt."
        action={
          <button
            onClick={openCreate}
            style={{ ...primaryButtonStyle, display: 'flex', alignItems: 'center', gap: '7px' }}
          >
            <Plus size={15} strokeWidth={2.5} /> Identiteit toevoegen
          </button>
        }
      >
        <Card padding="sm">
          {loading && (
            <p style={{ textAlign: 'center', padding: '40px 0', color: '#9CA3AF', fontSize: '14px' }}>
              Laden…
            </p>
          )}
          {fetchError && <div style={errorBoxStyle}>{fetchError}</div>}
          {!loading && !fetchError && (
            <IdentityTable
              identities={identities ?? []}
              onEdit={openEdit}
              onDelete={openDelete}
            />
          )}
        </Card>
      </PageShell>

      {/* Create */}
      <Modal open={mode === 'create'} title="Identiteit toevoegen" onClose={closeModal} maxWidth={560}>
        <IdentityForm
          onSubmit={handleCreate}
          onCancel={closeModal}
          loading={saving}
          error={formError}
          submitLabel="Identiteit aanmaken"
        />
      </Modal>

      {/* Edit */}
      <Modal open={mode === 'edit'} title="Identiteit bewerken" onClose={closeModal} maxWidth={560}>
        <IdentityForm
          initial={initialValues}
          onSubmit={handleEdit}
          onCancel={closeModal}
          loading={saving}
          error={formError}
          submitLabel="Wijzigingen opslaan"
        />
      </Modal>

      {/* Delete confirmation */}
      <Modal open={mode === 'delete'} title="Identiteit verwijderen" onClose={closeModal} maxWidth={420}>
        <p style={{ fontSize: '14px', color: '#374151', marginBottom: '6px' }}>
          Weet u zeker dat u <strong>{selected?.name}</strong> wilt verwijderen?
        </p>
        <p style={{ fontSize: '13px', color: '#9CA3AF', marginBottom: '24px' }}>
          Deze actie kan niet ongedaan worden gemaakt. Identiteiten die zijn gekoppeld aan actieve contracten of facturen kunnen niet worden verwijderd.
        </p>
        {formError && <div style={{ ...errorBoxStyle, marginBottom: '16px' }}>{formError}</div>}
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
          <button
            onClick={closeModal}
            style={{ padding: '10px 20px', borderRadius: '6px', fontSize: '14px', background: 'none', border: '1px solid #E5E7EB', color: '#374151', cursor: 'pointer' }}
          >
            Annuleren
          </button>
          <button
            onClick={handleDelete}
            disabled={saving}
            style={{ padding: '10px 20px', borderRadius: '6px', fontSize: '14px', fontWeight: 600, border: 'none', backgroundColor: '#DC2626', color: '#FFFFFF', cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.6 : 1 }}
          >
            {saving ? 'Verwijderen…' : 'Verwijderen'}
          </button>
        </div>
      </Modal>
    </>
  )
}
